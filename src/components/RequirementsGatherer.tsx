...
// Trigger Step Configuration Component - Chat-based interface matching RequirementsGatherer layout
function TriggerStepConfig({
  step,
  workflowId,
  workflowName,
  stepIndex,
  ...
  const [requirementsText, setRequirementsText] = useState(step.requirements?.requirementsText || '')
  ...
  useEffect(() => {
    if (step.requirements?.chatHistory && step.requirements.chatHistory.length > 0) {
      ...
      setBlueprint(step.requirements.blueprint || { greenList: [], redList: [] })
    }
  }, [step])
    ...
    if (!hasExistingHistory) {
      const sendInitialMessage = async () => {
        ...
        try {
          ...
          // Generate initial message AND initial blueprint in parallel
          const [initialMessage, initialBlueprint] = await Promise.all([
            getInitialTriggerMessage(
              step,
            ...
            extractTriggerConfiguration(
              step,
              [], // Empty conversation history - just use Create a Task context
              createTaskConversation?.messages
            )
          ])
          ...
          setBlueprint(initialBlueprint.blueprint)
          setRequirementsText(initialBlueprint.requirementsText)
          
          const requirements = {
            isComplete: false,
            requirementsText: initialBlueprint.requirementsText,
            chatHistory: [systemMessage],
            blueprint: initialBlueprint.blueprint,
          }
          
          updateStepRequirements(workflowId, step.id, requirements)
        } catch (error) {
          console.error('Error sending initial trigger message:', error)
          // Fallback to empty blueprint if extraction fails
          setBlueprint({ greenList: [], redList: [] })
        } finally {
          setIsLoading(false)
        }
      }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Save chat history and configuration
      const requirements = {
        isComplete: false,
        requirementsText: configResult.requirementsText,
        chatHistory: updatedMessages,
        blueprint: configResult.blueprint,
      }
  ...
  const handleMarkComplete = () => {
    const requirements = {
      isComplete: true,
      requirementsText: requirementsText || step.requirements?.requirementsText || '',
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-sm text-gray-darker">Start configuring your trigger below</p>
              </div>
            ) : (
              <>
        ...
        <div className="w-96 border-l border-gray-lighter bg-gray-50 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            {/* Required Conditions Section (for trigger) */}
            <div>
              ...
              {blueprint && blueprint.greenList.length > 0 ? (
                <div className="space-y-2">
                  {blueprint.greenList.map((condition, idx) => (
                    <Card
                      key={idx}
                      variant="outlined"
                      className="p-3 bg-green-50 border-green-200"
                    >
                      <p className="text-sm text-green-900">{condition}</p>
                    </Card>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-gray-darker italic">No required conditions defined yet...</p>
              )}
            </div>

            {/* Exceptions Section (yellow for trigger) */}
            <div>
              <div className="flex items-center gap-2 mb-3">
              ...
              {blueprint && blueprint.redList.length > 0 ? (
                <div className="space-y-2">
                  {blueprint.redList.map((exception, idx) => (
                    <Card
                      key={idx}
                      variant="outlined"
                      className="p-3 bg-yellow-50 border-yellow-200"
                    >
                      <p className="text-sm text-yellow-900">{exception}</p>
                    </Card>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-gray-darker italic">No exceptions defined yet...</p>
              )}
            </div>
          ...
          {blueprint && (
            <div className="p-4 border-t border-gray-lighter bg-white">
              <Button
                ...
                className="w-full"
              >
                Mark Trigger Configuration Complete
              </Button>
            </div>
          )}
        </div>
...
// End Step Configuration Component - Chat-based interface matching RequirementsGatherer layout
function EndStepConfig({
  step,
  workflowId,
  workflowName,
  stepIndex,
    ...
    if (!hasExistingHistory) {
      const sendInitialMessage = async () => {
        ...
        try {
          ...
          console.error('Error sending initial end message:', error)
          // Fallback to empty blueprint if extraction fails
          setBlueprint({ greenList: [], redList: [] })
        } finally {
          setIsLoading(false)
    ...
    <div className="flex flex-col h-screen bg-white">
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-sm text-gray-darker">Start configuring your end step below</p>
              </div>
            ) : (
              <>
                {messages.map((message, index) => (
          ...
          <div className="p-4 border-t border-gray-lighter bg-white">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={plusMenuRef}>
                ...
                {showPlusMenu && (
                  <div className="absolute bottom-full left-0 mb-2 bg-white border border-gray-lighter rounded-md shadow-lg z-10 min-w-48">
                    <button
                      onClick={() => {
                        document.getElementById('file-upload-end')?.click()
                        setShowPlusMenu(false)
                      }}
                      className="w-full px-4 py-2 text-left text-sm text-gray-dark hover:bg-gray-lighter flex items-center gap-2"
                    >
              ...
              <Input
                ...
                onChange={(e) => setInputValue(e.target.value)}
                onKeyPress={handleKeyPress}
                placeholder="Describe what happens when this workflow completes..."
                disabled={isLoading}
                className="flex-1"
              />
        ...
        <div className="w-96 border-l border-gray-lighter bg-gray-50 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            {/* Completion Criteria Section (for end) */}
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-darker">
              ...
              {blueprint && blueprint.greenList.length > 0 ? (
                <div className="space-y-2">
                  {blueprint.greenList.map((criterion, idx) => (
                    <Card
                      key={idx}
                      variant="outlined"
                      className="p-3 bg-green-50 border-green-200"
                    >
                      <p className="text-sm text-green-900">{criterion}</p>
                    </Card>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-gray-darker italic">No completion criteria defined yet...</p>
              )}
            </div>

            {/* Exceptions Section (yellow for end) */}
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-darker">
          ...
          {blueprint && (
            <div className="p-4 border-t border-gray-lighter bg-white">
              <Button
                ...
                className="w-full"
              >
                Mark End Step Configuration Complete
              </Button>
            </div>
          )}
        </div>
...
export default function RequirementsGatherer({
  ...
  // Check if this is a trigger or end step - show appropriate config UI
    ...
    if (!hasExistingHistory) {
      const sendInitialMessage = async () => {
        ...
        try {
          ...
          // Save initial message and blueprint to step requirements
          const requirements = {
            isComplete: false,
            requirementsText: initialBlueprint.requirementsText,
            chatHistory: [systemMessage],
            ...
            customRequirements: initialBlueprint.customRequirements || [],
            blueprint: initialBlueprint.blueprint,
          }
